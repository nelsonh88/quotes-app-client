...
var onShowAllUsers = function onShowAllUsers(event) {
  ...
  // }
  // else { data = '' }
  api.index().then(ui.allUsersSuccess).catch(ui.allUserFailure);
};
...
var apiUrl = void 0;
var apiUrls = {
  production: 'https://quotes-log-app.herokuapp.com/',
...
var userTemplate = __webpack_require__(15);
...
var signUpSuccess = function signUpSuccess(data) {
  ...
  goTop();
  $('form').trigger('reset');
  var newLede = 'Welcome ' + data.user.first_name + '! Thank you for signing up. Please sign in to let MotivationQ motivate you!';
  $('.lede').text(newLede);
};
...
var allUsersSuccess = function allUsersSuccess(data) {
  var showUsersQuotesHtml = userTemplate({ users: data.users });
  $('#content').html(showUsersQuotesHtml);
  $('.signed-in .add-quote').show();
};

var allUsersFailure = function allUsersFailure(data) {
  generateMessage('Error on retrieving quotes', 'danger');
};

var welcomeText = function welcomeText(data) {
  var newLede = 'Welcome ' + data.user.first_name + '! Below you will find options to get your quotes and create quotes. You also have access to see what other users have posted for their quotes and if you like their quotes add them to your quotes!';
  $('.lede').text(newLede);
};

var byeText = function byeText() {
  var newLede = 'Thank you, you have been successfully logged out. Stay motivated!!';
  $('.lede').text(newLede);
};
...
var quoteEvents = __webpack_require__(32);
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "\n      <div class=\"user\">\n        <span>Submitted by</span>\n        <a href=\"javascript:void(0)\">"
    ...
    + "</a>\n      </div>\n"
    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),((stack1 = blockParams[0][0]) != null ? stack1.quotes : stack1),{"name":"each","hash":{},"fn":container.program(2, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
},"2":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return "    <div class=\"quote-block\">\n"
    + ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),blockParams[0][0],{"name":"if","hash":{},"fn":container.program(3, data, 0, blockParams),"inverse":container.program(5, data, 0, blockParams),"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "    </div>\n";
},"3":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1, alias1=container.lambda, alias2=container.escapeExpression;

  return "      <button class=\"add-quote btn btn-dark\" data-quote=\""
    + alias2(alias1(((stack1 = blockParams[1][0]) != null ? stack1.quote : stack1), depth0))
    + "\" data-author=\""
    + alias2(alias1(((stack1 = blockParams[1][0]) != null ? stack1.author : stack1), depth0))
    + "\" data-id="
...
var onCreateQuote = function onCreateQuote(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.createQuote(data).then(ui.createQuoteSuccess).then(api.indexQuotes).then(ui.indexQuotesSuccessNoMessage).catch(ui.createQuoteFailure);
};

var onIndexQuotes = function onIndexQuotes(event) {
  event.preventDefault();
  api.indexQuotes().then(ui.indexQuotesSuccess).catch(ui.indexQuotesFailure);
};

var onAllQuotes = function onAllQuotes(event) {
  event.preventDefault();
  authEvents.onShowAllUsers();
  $('.user').show();
  $('#all-quotes').hide();
...
var onShowQuote = function onShowQuote(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  var quote = data.quote;
  // console.log(event)
  api.showQuote(quote.id).then(ui.showQuoteSuccess).catch(ui.showQuoteFailure);
};
...
var onUpdateQuote = function onUpdateQuote(event) {
  ...
  api.updateQuote(data, id).then(ui.updateQuoteSuccess).then(api.indexQuotes).then(ui.indexQuotesSuccessNoMessage).catch(ui.updateQuoteFailure);
};
...
var onDeleteQuote = function onDeleteQuote(event) {
  ...
  var id = event.target.dataset.id;
  var quote = data.quote;
  api.deleteQuote(id).then(ui.deleteQuoteSuccess).then(api.indexQuotes).then(ui.indexQuotesSuccessNoMessage).catch(ui.deleteQuoteFailure);
};

var onAddQuote = function onAddQuote(event) {
  event.preventDefault();
  var data = { quote: { quote: event.target.dataset.quote, author: event.target.dataset.author } };
  api.addQuote(data).then(ui.addQuoteSuccessful)
  // .then(() => onCreate(event))
...
var indexQuotes = function indexQuotes() {
  return $.ajax({
    url: config.apiUrl + '/quotes',
    method: 'GET',
    headers: {
...
var showQuote = function showQuote(id) {
  return $.ajax({
...
var updateQuote = function updateQuote(data, quoteid) {
  return $.ajax({
...
var pageMessage = $('#message');
var ownedQuotesTemplate = __webpack_require__(35);
var store = __webpack_require__(3);
// above is for the token as well
...
var createQuoteFailure = function createQuoteFailure() {
  generateMessage('Error On Creating Quote!', 'danger');
  $('form').trigger('reset');
  goTop();
};

var indexQuotesSuccess = function indexQuotesSuccess(data) {
  generateMessage('Got Your Quotes Successfully!', 'success');
  $('.show-my-quotes').hide();
  $('#index-quotes').hide();
  $('#all-quotes').show();
  $('.create-a-quote').show();
  var showUsersOwnedQuotesHtml = ownedQuotesTemplate({ quotes: data.quotes });
  $('#content').html(showUsersOwnedQuotesHtml);
  goTop();
};

var indexQuotesFailure = function indexQuotesFailure(error) {
  generateMessage('Error On Getting Your Quotes!', 'danger');
  console.error(error);
  goTop();
};

var showQuoteSuccess = function showQuoteSuccess(data) {
  generateMessage('Retieved A Quote Successfully!', 'success');
};

var showQuoteFailure = function showQuoteFailure() {
...
var indexQuotesSuccessNoMessage = function indexQuotesSuccessNoMessage(data) {
  var showUsersOwnedQuotesHtml = ownedQuotesTemplate({ quotes: data.quotes });
  $('#content').html(showUsersOwnedQuotesHtml);
};

var addQuoteSuccessful = function addQuoteSuccessful(data) {
...
module.exports = {
  ...
  indexQuotesSuccess: indexQuotesSuccess,
  indexQuotesFailure: indexQuotesFailure,
  showQuoteSuccess: showQuoteSuccess,
  showQuoteFailure: showQuoteFailure,
  ...
  indexQuotesSuccessNoMessage: indexQuotesSuccessNoMessage,
  addQuoteSuccessful: addQuoteSuccessful,
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "<div class=\"quote-block\" data-id="
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + ">\n  <button class=\"btn btn-danger delete-quote\" data-id="
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + ">Delete</button>\n  <!-- <p class=\"quoteid\">ID: "
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "</p> -->\n  <button type=\"button\" class=\"update-quote btn btn-info update-quote-button\" data-id="
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + " data-target=\"#update-quote-modal\" data-toggle=\"modal\">Update</button>\n  <p class=\"quote\">"
...
module.exports = __webpack_require__.p + "cabc60c547113047cb8d1a1c6b874331.jpg";
